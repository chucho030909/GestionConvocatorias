...
import api, { obtenerMiProyectoEnConvocatoria, obtenerAvancesPorProyecto, crearAvance, descargarArchivo, crearRepositorio, abrirArchivoConvocatoria } from '../services/api';
...
function DocumentoBoton({ convocatoriaId, tipo, label }) {
  if (!convocatoriaId || !tipo) return <span className="text-sm text-gray-400 italic">No disponible</span>;
  return (
    <button
      type="button"
      onClick={() => abrirArchivoConvocatoria(convocatoriaId, tipo)}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="space-y-3">
          <div>
            ...
            <DocumentoBoton convocatoriaId={convocatoria.id} tipo={convocatoria.rutaBases ? 'bases' : null} label="Bases de la convocatoria" />
          </div>
          <div>
            <label className="text-xs text-gray-400 uppercase tracking-wide mb-1 block">Convocatoria PDF</label>
            <DocumentoBoton convocatoriaId={convocatoria.id} tipo={convocatoria.rutaConvocatoriaPDF ? 'convocatoria' : null} label="Documento de convocatoria" />
          </div>
          <div>
            <label className="text-xs text-gray-400 uppercase tracking-wide mb-1 block">Formatos</label>
            <DocumentoBoton convocatoriaId={convocatoria.id} tipo={convocatoria.rutaFormatos ? 'formatos' : null} label="Formatos descargables" />
          </div>
        </div>
      </div>
    </div>
