...
{
    public static async Task SeedConvocatoriaConsecutivosAsync(AppDbContext context)
    {
        var claves = await context.Convocatorias
            .AsNoTracking()
            .Select(c => c.Clave)
            .ToListAsync();

        var pattern = new Regex(@"^CONV-(\d{4})-(\d{2,4})$", RegexOptions.Compiled);
        var porAnio = new Dictionary<int, int>();

        foreach (var clave in claves)
        {
            var match = pattern.Match(clave);
            if (match.Success && int.TryParse(match.Groups[1].Value, out var anio) && int.TryParse(match.Groups[2].Value, out var numero))
            {
                if (!porAnio.ContainsKey(anio) || porAnio[anio] < numero)
                    porAnio[anio] = numero;
            }
        }

        foreach (var kv in porAnio)
        {
            var existente = await context.ConvocatoriaConsecutivos.FindAsync(kv.Key);
            if (existente == null)
            {
                context.ConvocatoriaConsecutivos.Add(new ConvocatoriaConsecutivo
                {
                    Anio = kv.Key,
                    UltimoNumero = kv.Value
                });
            }
            else if (existente.UltimoNumero < kv.Value)
            {
                existente.UltimoNumero = kv.Value;
                context.ConvocatoriaConsecutivos.Update(existente);
            }
        }
